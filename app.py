...
def send_email(to, subject, html_content):
    """Kirim email menggunakan Brevo (Sendinblue) API."""
    
    # Ambil API Key dari environment variables (diasumsikan nama variabelnya BREVO_API_KEY)
    api_key = os.environ.get('BREVO_API_KEY') 
    sender_email = app.config['MAIL_DEFAULT_SENDER'] # Tetap gunakan sender dari config
    
    if not api_key:
        print("❌ ERROR: BREVO_API_KEY tidak ditemukan di environment.")
        raise Exception("Configuration Error: BREVO_API_KEY is missing.")
    ...
    send_smtp_email = sib_api_v3_sdk.SendSmtpEmail(
        ...
        sender={"email": sender_email, "name": "Geboy Mujair Admin"}, # Tambahkan nama pengirim
        subject=subject
    )

    try:
        # Panggilan API ini menggunakan HTTPS (Port 443), yang diizinkan Railway
        api_response = api_instance.send_transac_email(send_smtp_email)
        print(f"✅ Email sent via Brevo. Response: {api_response}")
        return True
        
    except ApiException as e:
        # Menangkap error jika sender belum diverifikasi atau API Key salah
        error_msg = f"Brevo API Error: {e.status} - {e.reason} - {e.body}"
        print(f"❌ FATAL ERROR in send_email (Brevo): {error_msg}")
        raise Exception(error_msg)
    except Exception as e:
        # Menangkap error Python lainnya
        print(f"❌ Exception di send_email (Brevo): {e}")
        raise e
